...
if (!all(c("ad_sector_codes", "ad_sector_names") %in% names(sector_lookup))) {
  stop("❌ Sector lookup missing expected columns.")
}
...
if (!require("pacman")) install.packages("pacman")
pacman::p_load(tidyverse, janitor, gt, readr, stringr)
...
if (length(files) == 0) stop("❌ No dCdH files found — check path or filenames.")
...
if (!all(c("ad_sector_codes", "ad_sector_names") %in% names(sector_lookup))) {
  stop("❌ Sector lookup missing required columns.")
}
...
cat("\n✅ Cleaned Parallel Trends (dCdH) summary preview:\n")
print(placebo_df)
...
if (length(files) == 0) stop("❌ No TWFE files found — check path.")
...
cat("\n✅ Post-treatment estimates preview:\n")
print(head(twfe_all))
...
cat("\n✅ Post-treatment effect rows preview:\n")
...
if (!require("pacman")) install.packages("pacman")
pacman::p_load(tidyverse, gt, janitor, readr)
...
cat("\n✅ Distance-band coefficients preview:\n")
print(band_filtered)
...
cat("\n✅ Formatted Moran's I table preview:\n")
...
read_one_twfe_robust <- function(f) {
  ...
  # Identify columns
  time_col <- names(df)[str_detect(names(df), "time|term|x1")][1]
  est_col <- names(df)[str_detect(names(df), "estimate|est|x2")][1]
  low_col <- names(df)[str_detect(names(df), "low|conf_l|x3")][1]
  high_col <- names(df)[str_detect(names(df), "high|conf_h|x4")][1]

  df %>%
    mutate(
      time_num = as.numeric(str_extract(.data[[time_col]], "-?[0-9]+")),
...
cat("\n✅ Table A6 saved successfully as HTML in:", save_path, "\n")
print(table_a6)
...
if (!file.exists(dcdh_file)) {
  cat("\n⚠️ dCdH results file not found. Run dcdh_with_covariates.R first.\n")
